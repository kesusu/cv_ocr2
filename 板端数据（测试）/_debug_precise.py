...
def rect_nms(boxes, scores, iou_t=0.5, min_sc=None):
    ...
    order=sorted(range(n),key=lambda i:scores[i],reverse=True);keep=[]
...
img=load_image('photos/photo_1.jpg'); h,w=img.shape[:2]; TARGET=88; TILE=640; OV=0.20; TH=0.25; BT=0.40

print(f"Multi-tile detection...")
...
# 精确扫描 0.695 ~ 0.700, 步长0.001
print(f"\n{'ScoreThresh':>10s} {'Boxes':>6s} {'Score':>7s} {'Gap'}")
print("-"*38)
best_diff=9999; best_cfg=None; best_cnt=0; best_avg=0
for sc_t in np.arange(0.694, 0.702, 0.001):
    mb,ms=rect_nms(raw_b,raw_s,min_sc=float(round(sc_t,3)))
    cnt=len(mb); avg=float(np.mean(ms))if ms else 0; diff=cnt-TARGET
    marker=""
    if abs(diff)<abs(best_diff):best_diff,best_cfg,best_cnt,best_avg=abs(diff),round(sc_t,3),cnt,avg
    if cnt>=85 and cnt<=93 or abs(diff)<=3:
        if diff==0:marker=" <<<PERFECT"
        elif abs(diff)<=1:marker=" <<<TARGET"
        elif abs(diff)<=3:marker=" ***CLOSE"
    print(f"  {sc_t:>8.3f}   {cnt:>6d} {avg:>7.3f} {diff:+4d}{marker}")

print(f"\n{'='*45}")
print(f"OPTIMAL: sc>{best_cfg:.3f} => {best_cnt} boxes(score={best_avg:.3f},gap={best_cnt-TARGET:+d})")

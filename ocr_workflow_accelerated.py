...
def main():
    ...
    try:
        if MODE == 2:
            print("\n[模式2] 摄像头拍照识别")
            # 摄像头模式 (简化版, 主要用于图片模式)
            print("  注意: 加速版主要用于图片批量测试, 摄像头模式请使用原版")
        elif MODE == 1:
            if not IMAGE_PATHS or all(p.strip().startswith('#') for p in IMAGE_PATHS):
                print("!!! 配置区 IMAGE_PATHS 为空，将使用测试图片 !!!")
                test_paths = [p for p in TEST_IMAGE_PATHS
                              if p and not p.strip().startswith('#') and os.path.exists(p)]
                if test_paths:
                    run_test_and_save_report(
                        engine, test_paths,
                        os.path.join(BASE_DIR, 'ocr_test_result.txt')
                    )
                else:
                    print("!!! 测试图片也不存在 !!!")
                return
